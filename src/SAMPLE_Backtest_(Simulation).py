...
num_sig = 0
setting_sig_cal = {
    "cash_safety_perc": np.float64(
        round(1.00 / 100.00, 5)
    ),  # 0.00 is fine because we prepare for the slippage in the main strategy cal
    "allow_negative_cash": True,
    "buy_back": True,
    "trading_value_max_perc": np.float64(round(10000000000000000000.00 / 100.00, 5)),  #
    "trading_value_max_exe_perc": np.float64(
        round(10000000000000000.00 / 100.00, 5)
    ),  #
}
...
trading_value_min = trading_value_s.apply(
    lambda col: np.minimum(col, trading_value_m[col.name])
).astype(np.float64)
...
max_share_to_trade = (
    trading_value_min / c * setting_sig_cal["trading_value_max_exe_perc"]
)
...
target_weight_ = target_weight[
    setting_backtest["start_date"] : setting_backtest["end_date"]
].shift()
...
rebalance_trigger = (
    (target_weight_ != target_weight_.shift()).astype(np.float64).sum(axis=1) > 0
).to_numpy()
target_weight_ = target_weight_.to_numpy()


c_range = (
    c.ffill()[setting_backtest["start_date"] : setting_backtest["end_date"]]
).to_numpy()
exe_price = o.ffill()[
    setting_backtest["start_date"] : setting_backtest["end_date"]
].to_numpy()
...
for index, date_i in enumerate(datetime_list, start=0):
    ...
    sell_txn_value = np.nansum(
        shares_sell_done * exe_price_i_1 * (1 - setting_backtest["slip_pec"])
    )
    buy_txn_value = np.nansum(
        shares_buy_done * exe_price_i_1 * (1 + setting_backtest["slip_pec"])
    )
    ...
    if index == (len(datetime_list) - 1):
        print(
            ">>>  yesterday realized cash (% NAV): "
            + str((total_pnl_i_1 * 100) / NAV_i_1_end)
        )
    ...
    if rebalance_trigger[index]:
        ...
        if setting_sig_cal["buy_back"] == False:
            buy_exe = ((share_i_0 > share_i_1) & ((share_i_1 - 1) < 1)).astype(
                np.float64
            )
        sell_exe = (share_i_0 < share_i_1).astype(np.float64)
        hold = (share_i_0 == share_i_1).astype(np.float64)

        port_exe_shares_buy[index, :] = np.nan_to_num(
            (buy_exe * (share_i_0 - share_i_1)), nan=0.0000
        ).astype(np.int64)
        port_exe_shares_sell[index, :] = np.nan_to_num(
            (sell_exe * (share_i_1 - share_i_0)), nan=0.0000
        ).astype(np.int64)

        max_share_trade_i = max_share_to_trade[index, :]
        port_exe_shares_buy[index, :] = np.nan_to_num(
            np.minimum(port_exe_shares_buy[index, :], max_share_trade_i), nan=0.0000
        ).astype(np.int64)
        if False:
            # want to sell otherwise we don't have sufficient cash
            port_exe_shares_sell[index, :] = np.nan_to_num(
                np.minimum(port_exe_shares_sell[index, :], max_share_trade_i),
                nan=0.0000,
            ).astype(np.int64)

        port_share[index, :] = (
            share_i_1
            + port_exe_shares_buy[index, :].astype(np.int64)
            - port_exe_shares_sell[index, :].astype(np.int64)
        )
        a = index
...
port_share = pd.DataFrame(
    index=dataframe_blueprint.index,
    columns=dataframe_blueprint.columns,
    data=port_share,
)
port_exe_shares_buy = pd.DataFrame(
    index=dataframe_blueprint.index,
    columns=dataframe_blueprint.columns,
    data=port_exe_shares_buy,
)
port_exe_shares_sell = pd.DataFrame(
    index=dataframe_blueprint.index,
    columns=dataframe_blueprint.columns,
    data=port_exe_shares_sell,
)
c_range = pd.DataFrame(
    index=dataframe_blueprint.index, columns=dataframe_blueprint.columns, data=c_range
)
...
ex_date = pd.pivot_table(
    pay_dividend_local, columns="symbol", index="ex_date", values="dps"
).reindex(c.index)
pay_date = pd.pivot_table(
    pay_dividend_local, columns="symbol", index="pay_date", values="dps"
).reindex(c.index)
div_comb = data_exist_1_nan(ex_date.shift()).replace(np.nan, 0) + data_exist_1_nan(
    pay_date
).replace(np.nan, 0)
...
final_to_buy_share = (
    port_exe_shares_buy.iloc[-1].replace(0, np.nan).dropna().astype(np.int32)
)
final_to_sell_share = (
    port_exe_shares_sell.iloc[-1].replace(0, np.nan).dropna().astype(np.int32)
)
final_weight = (
    ((port_share.iloc[-1] * c_range.iloc[-2]).replace(0, np.nan).dropna()) / final_NAV
).round(3)
...
print(
    "Daily Model: "
    + str(round((port_NAV / port_NAV.shift() - 1).round(4).iloc[-2].item(), 4) * 100.0)
    + "%"
)
...
curve_cum["distributed"] = (
    paid_date_series.replace(0, np.nan).reindex(port_NAV.index)
) * port_NAV.iloc[:, 0].values
...
ax.plot(
    curve_cum["distributed"].index,
    curve_cum["distributed"].values,
    marker=".",
    color="green",
)
ax.set_yscale("log")
...
if True:
    ...
    bar_return, turnover = thana_backtester.gen_bar_return(
        port_share, setting_backtest, c.ffill(), o.ffill()
    )
    import ThaiEQ_strat_tool.backtest as thana_backtester
    ...
    (
        ...
        yearly_MaxDD_dep,
        NAV_change,
    ) = thana_backtester.run_backtest(bar_return, setting_backtest)
    ...
    print(
        (
            port_NAV.iloc[-1]
            - (NAV_change + setting_backtest["initial_deposit"]).iloc[-2]
        )
        / NAV_change.iloc[-2]
        * 100
    )
...
(
    summary_txn_by_timeline,
    summary_txn_by_order,
    summary_txn_dict,
) = thana_backtester.get_summary_txn_pct(port_share, bar_return, o, setting_backtest)
