...
def matplot_fig_to_img(fig, format_="PNG"):

    import io

    buf = io.BytesIO()
    fig.savefig(buf, format=format_)
    buf.seek(0)
    im = Image.open(buf)
    return im


def matplot_fig_to_bytes(fig, format_="PNG"):
...
pic2 = g2.fig
pic1.show()
im1 = matplot_fig_to_img(pic1, 'PNG')
im2 = matplot_fig_to_img(pic1, 'PNG')

img_new = get_concat_v_resize(img_new, img_new, resize_big_image=False)
img_new.show()
"""
