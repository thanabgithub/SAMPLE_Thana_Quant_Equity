...
def setup_benchmark(setting_backtest_: dict):
    if(type(setting_backtest_['start_date'])!=str):
        setting_backtest_['start_date'] = setting_backtest_['start_date'].strftime("%Y-%m-%d")
    if(type(setting_backtest_['end_date'])!=str):
        setting_backtest_['end_date'] = setting_backtest_['end_date'].strftime("%Y-%m-%d")
    ...
    try:
        year_range = [*range(int(setting_backtest_['start_date'].strftime("%Y-%m-%d")[0:4]), int(setting_backtest_['end_date'].strftime("%Y-%m-%d")[0:4])+1, 1)]
    except:
        year_range = [*range(int(setting_backtest_['start_date'][0:4]), int(setting_backtest_['end_date'][0:4])+1, 1)]
    ...
    benchmark_new_high = benchmark_series[setting_backtest_['start_date']:setting_backtest_['end_date']].cummax()
    ...
    benchmark_drawdown = benchmark_drawdown.reindex(pd.DatetimeIndex(benchmark_drawdown.index))
    benchmark_series = benchmark_series.reindex(pd.DatetimeIndex(benchmark_series.index))
    ...
    for year_int in year_range:
        yearly_bechmark_return[year_int] = (benchmark_series[str(year_int)][-1] - benchmark_series[str(year_int)][0])/benchmark_series[str(year_int)][0]*100.0
...
def gen_bar_return(target_shares_: pd.DataFrame, setting_backtest_: dict, c_: pd.DataFrame, exe_: pd.DataFrame):
    ...
    trade = (target_shares_.diff() != 0).fillna(0)*target_shares_.diff()*exe_price_df
    trade.iloc[0] = 0
    commission_fee = trade.abs()*commission_fee_by_perc
  
    bar_return_ = ((pnl_base_hld + pnl_base_trd) - commission_fee).replace(np.nan, 0)
    turnover_ = ( (trade.abs()[setting_backtest_['start_date']:setting_backtest_['end_date']].cumsum().iloc[-1]/2.00).sum()  )/setting_backtest_['initial_deposit']     
    return bar_return_, turnover_ # , exe_, exe_price_df

def run_backtest(bar_return_: pd.DataFrame, setting_backtest_: dict):
    NAV_change = bar_return_[setting_backtest_['start_date']:setting_backtest_['end_date']].sum(axis = 1).cumsum()
    ...
    new_high = NAV[setting_backtest_['start_date']:setting_backtest_['end_date']].cummax()
    ...
    #         ax.set_ylim([float(NAV.min()*.999), float(NAV.max()*1.05)])     # NAV.iloc[-1].item()
        try:
            benchmark_series = setting_backtest_['benchmark']['PTT'][setting_backtest_['start_date']:setting_backtest_['end_date']]       
        except:
            benchmark_series = setting_backtest_['benchmark'].iloc[:, 0][setting_backtest_['start_date']:setting_backtest_['end_date']] 
        benchmark_series = benchmark_series.reindex(pd.DatetimeIndex(benchmark_series.index))            
        benchmark_series = benchmark_series/(benchmark_series.iloc[0])*setting_backtest_['initial_deposit']     
        ax.plot(benchmark_series.index, benchmark_series.values, color="black", label = 'benchmark', linewidth=0.50)    
        ax.tick_params('y', colors = 'blue')
        # twin object for two different y-axis on the sample plot
        ax2=ax.twinx()
        # make a plot with different y-axis using second axis object
        if(setting_backtest_['mode'] == 'dynamic'):
            ax2.plot(drawdown_percent_NAV.index, drawdown_percent_NAV.values,color="red", label = 'DD_NAV', alpha=0.3)           
        elif(setting_backtest_['mode'] == 'static'):
            ax2.plot(drawdown_percent_dep.index, drawdown_percent_dep.values,color="red", label = 'DD_dep', alpha=0.3)
        ...
        if (setting_backtest_['save_fig']):    
            plt.savefig(setting_backtest_['report_dir'] + '\\NAV.png',bbox_inches='tight',dpi=100)
    ...
    if(type(setting_backtest_['end_date']) == str):     
        ...
        end_date = datetime.strptime(setting_backtest_['end_date'], '%Y-%m-%d')  # datetime.datetime(2010,1,1)
    ...
    for year_int in year_range:
        if(setting_backtest_['mode'] == 'dynamic'):
            yearly_return[year_int] = bar_return_[str(year_int)].sum()/prev_year_NAV[str(year_int)].iloc[0].item()*100.00
        elif(setting_backtest_['mode'] == 'static'):
            yearly_return[year_int] = bar_return_[str(year_int)].sum()/setting_backtest_['initial_deposit']*100.00
    ...
    yearly_sd_np = (ta.STDDEV(NAV_daily_return_np, len(NAV_daily_return_np))[-1])*((bar_per_year)**(1./2.))
    ...
    for year_int in year_range:
        if(setting_backtest_['mode'] == 'dynamic'):
            yearly_MaxDD_dep[year_int] = drawdown[str(year_int)].min()/new_high[str(year_int)].iloc[0].item()*100.00  
        elif(setting_backtest_['mode'] == 'static'):
            yearly_MaxDD_dep[year_int] = drawdown[str(year_int)].min()/setting_backtest_['initial_deposit']*100.00 
    ...
    yearly_benchmark_return = pd.DataFrame.from_dict(setting_backtest_['yearly_benchmark_return'], orient = 'index') 
    yearly_bechmark_drawdown = pd.DataFrame.from_dict(setting_backtest_['yearly_benchmark_MaxDD'], orient = 'index') 
    ...
    color_dict = {'model_profit': 'darkgreen', 'model_MaxDD_dep': 'red', 'benchmark_profit': 'cornflowerblue', '_': 'grey', 'benchmark_MaxDD': 'orange'}
    # color_list = ['green', 'red', 'blue', 'orange']
    if (setting_backtest_['display']):
        # yearly_profit_MaxDD_dep.plot(kind="bar", legend=True, colors = color_list)
        yearly_profit_MaxDD_dep.plot(color=[color_dict.get(x, '#333333') for x in yearly_profit_MaxDD_dep.columns], kind="bar", legend=True)
        ...
        if (setting_backtest_['save_fig']):    
            plt.savefig(setting_backtest_['report_dir'] + '\\profit_bar.png',bbox_inches='tight',dpi=100)
    ...
    ## cal summary    
    total_return = round((NAV.iloc[-1].item()/setting_backtest_['initial_deposit'] - 1.00)*100, 2)
    annual_return = round(total_return/num_months*12.0, 2)  

    if(setting_backtest_['mode'] == 'dynamic'):
        annual_return = (NAV.iloc[-1].item()/setting_backtest_['initial_deposit'])**(12/(num_months))-1
...
def get_summary_txn(target_shares_: pd.DataFrame, bar_return_: pd.DataFrame, setting_backtest_: dict):
    ...
    for stock in summary_txn_stock:
        ...
        for row in range(1, len(summary_txn_mini), 1):
            ...
            if((summary_txn_mini.iloc[row] > 0.0) and (summary_txn_mini.iloc[row - 1] == 0.0)):
                txn_dict[txn_num]= {}
            ...
            if((summary_txn_mini.iloc[row] == 0.0) and (summary_txn_mini.iloc[row - 1] > 0.0)):
                ...
                txn_dict[txn_num]['profit'] = bar_return_mini[txn_dict[txn_num]['entry_date']:txn_dict[txn_num]['exit_date']].sum()     
    ...
    summary_txn_by_timeline_.to_csv(setting_backtest_['report_dir'] + '\\summary_txn_by_timeline.csv')
    summary_txn_by_order_.to_csv(setting_backtest_['report_dir'] + '\\summary_txn_by_order.csv') 
                                   
    return summary_txn_by_timeline_, summary_txn_by_order_, txn_dict

def get_summary_txn_pct(target_shares_: pd.DataFrame, bar_return_: pd.DataFrame, o_: pd.DataFrame, setting_backtest_: dict):
    start_date_ = setting_backtest_['start_date']
    end_date_ = setting_backtest_['end_date']    

    summary_txn_by_timeline_ = target_shares_[start_date_:end_date_].copy().replace(0, np.nan)
    summary_value_by_timeline_ = (target_shares_*o_.ffill()[start_date_:end_date_]).replace(0, np.nan)
    ...
    for stock in summary_txn_stock:
        ...
        for row in range(1, len(summary_txn_mini), 1):
            ...
            if((summary_txn_mini.iloc[row] > 1.0) and (summary_txn_mini.iloc[row - 1] < 1.0) ):
                txn_dict[txn_num]= {}
                txn_dict[txn_num]['symbol'] = summary_txn_mini.name
                txn_dict[txn_num]['entry_date'] = index_[row]
                txn_dict[txn_num]['entry_price'] = execution_price[stock][row] 
                txn_dict[txn_num]['buy_value'] = execution_price[stock][row]*summary_txn_mini.iloc[row]
                # print(txn_dict[txn_num]['entry_value'])
            if txn_num in txn_dict.keys():
                if (summary_txn_mini.iloc[row] - summary_txn_mini.iloc[row-1] > 1) :
                # buy
                    txn_dict[txn_num]['buy_value'] = txn_dict[txn_num]['buy_value'] + execution_price[stock][row]*(summary_txn_mini.iloc[row] - summary_txn_mini.iloc[row - 1])
                
            if((summary_txn_mini.iloc[row] < 1.0) and (summary_txn_mini.iloc[row - 1] > 1.0)):
                
                txn_dict[txn_num]['exit_date'] = index_[row]   
                txn_dict[txn_num]['exit_price'] = execution_price[stock][row]  
                txn_dict[txn_num]['average_share'] = target_shares_[stock][txn_dict[txn_num]['entry_date']:txn_dict[txn_num]['exit_date']].mean() 
                txn_dict[txn_num]['net_pnl'] = bar_return_mini[txn_dict[txn_num]['entry_date']:txn_dict[txn_num]['exit_date']].sum()     
...
def backup_code(filename_: str, path_: str, time):
    ...
    report_directory_ = path_ + '\\report\\' + filename_  + '_' + str(folder_name_time_format)
