...
def EMA(series: NdType, periods: int = 20, warm_periods: tp.Union[int, None] = None) -> NdType:
    """
    Exponential moving average
    """
    if warm_periods is None:
        warm_periods = periods
    return nd_universal_adapter(ema_np_1d, (series,), (periods, warm_periods,))
...
def wilder_ma(series: NdType, periods: int = 14, warm_periods: tp.Union[int, None] = None):
    """
...
def MA_trim(input_: NdType, periods_, trim_ = 0.05):
    ...
    target_ = input_
    target_clean = clean_target(target_)
    output = nd_universal_adapter(MA_trim_series, (target_clean,), (periods_, trim_,))
    return output*exist
...
def get_downside_deviation(input_: NdType, periods_ = 60):
    ...
    target_ = input_
    target_clean = clean_target(target_)
    output = nd_universal_adapter(downside_deviation_series, (target_clean,), (periods_, ))
    return output*exist
...
def low_pass_filter_deseason(target_, train_cycle = 2, period_ = 14):
    ...
    return target_clean.apply(col_low_pass_filter_deseason, axis=0, args=(train_window, period_,))*exist
...
def low_pass_filter_detrend(target_, train_cycle = 2, period_ = 14):
    ...
    target_clean = clean_target(target_)
    train_window = train_cycle*period_
    return target_clean.apply(col_low_pass_filter_detrend, axis=0, args=(train_window, period_,))*exist
...
def rank_ts_old(data_df, window):
    return data_df.apply(lambda s: s.rolling(window).apply(lambda x: x.rank(pct=True).iloc[-1]), axis=0)
...
def correlation(target_: pd.DataFrame, SET_: pd.DataFrame, period_ = 60):
    ...
    for row in range(period_-1 ,bar_num ,1): 
        ...
        if(row%100 == 0):
            print(row)
    correlation_result_ = pd.DataFrame(index = target_.index, columns = target_.columns, data = correlation_result_np)
...
def series_to_df(mother_: pd.Series, df_blueprint_: pd.DataFrame):
    # dubplicated
    extended_df_ = pd.DataFrame(index = df_blueprint_.index, columns = df_blueprint_.columns)
...
def one_zero_gen_frm_BUYSELL(BUY_bool: pd.DataFrame, SELL_bool: pd.DataFrame, universe: pd.DataFrame) -> pd.DataFrame:
...
def ranking_ASC_within_universe(input_df: pd.DataFrame, universe: pd.DataFrame)->pd.DataFrame:
...
def ranking_DSC_within_universe(input_df: pd.DataFrame, universe: pd.DataFrame)->pd.DataFrame:
    universe.replace(0, np.nan, inplace = True) 
    target_ = input_df.copy()
    target_ = target_*universe
...
def ranking_ASC_within_universe_pct(input_df: pd.DataFrame, universe: pd.DataFrame)->pd.DataFrame:
    universe.replace(0, np.nan, inplace = True) 
    target_ = input_df.copy()
    target_ = target_*universe
...
def ranking_DSC_within_universe_pct(input_df: pd.DataFrame, universe: pd.DataFrame)->pd.DataFrame:
    universe.replace(0, np.nan, inplace = True) 
    target_ = input_df.copy()
    target_ = target_*universe
...
def universe_smoothold(universe_input, num_of_SET_member_series_, threshold_perc = 0.025):
    ...
    for row_index in range(1, bar_num):
        ...
        diff_series = (universe_input.iloc[row_index, :]!=universe_output.iloc[row_index, :]).astype(np.float64)
...
def universe_smooth(universe_input, num_of_SET_member_series_, threshold_perc = 0.025):
    ...
    for row_index in range(1, bar_num):
        ...
        diff_series = (universe_input_np[row_index, :]!=universe_output_np[row_index, :]).astype(np.float64)
    ...
    universe_output = pd.DataFrame(index = universe_input.index, columns = universe_input.columns, data = universe_output_np)
    return universe_output.replace(0, np.nan)
...
def VMAold(target_: pd.DataFrame, target_v_: pd.DataFrame, period_: int)->pd.DataFrame:
    period_ = int(period_)
    data_exist = (target_.isnull()==False).astype(np.float64).replace(0, np.nan)
    data_exist = data_exist*(data_exist.shift(period_))
    target_ = clean_target(target_)
    ...
    PV_period_sum =  PV.apply(lambda col: ta.SUM(col.astype(np.float64), period_)).astype(np.float64)
    V_period_sum = target_v_.apply(lambda col: ta.SUM(col.astype(np.float64), period_)).astype(np.float64)
...
def powerMA(target_: pd.DataFrame, last_step_ = 0.75, period_ = 10):
    ...
    for period_j in range(0, period_):
        ...
        if period_j%50 == 0:
            print(period_j)
        component_p =  (target_.shift(period_j) - target_.shift(period_j+1))/(base_**(period_-period_j))
...
def z_score_trim_sm(target_: pd.DataFrame, period_sm_:int, period_: int, trim = 0.05)->pd.DataFrame:
    period_ = int(period_)
    period_sm_ = int(period_sm_)
    data_exist = (target_.isnull()==False).astype(np.float64).replace(0, np.nan)
    data_exist = data_exist*(data_exist.shift(period_+period_sm_))
    target_ = clean_target(target_)
    SMA_tar = MA_trim(target_, period_, )
    STDDEV_tar = STDDEV(target_, period_)
    z_score_ = (MA(target_, period_sm_)  - SMA_tar)/STDDEV_tar
    z_score_.replace(np.inf, np.nan, inplace = True)
    z_score_.replace(np.nan, 0, inplace = True)
...
def ADX(h_: pd.DataFrame, l_: pd.DataFrame, c_: pd.DataFrame, period_: int)-> pd.DataFrame:
    period_ = int(period_)
    ...
    ADX_ = ADX_.apply(lambda col: ta.ADX(h_[col.name], l_[col.name], c_[col.name], period_)).astype(np.float64)
    ADX_.replace(np.inf, 0, inplace = True)
    ADX_.replace(np.nan, 0, inplace = True)
    ADX_ = ADX_*data_exist
    return ADX_    

def PDI(h_: pd.DataFrame, l_: pd.DataFrame, c_: pd.DataFrame, period_: int)-> pd.DataFrame:
    period_ = int(period_)
    data_exist = (c_.isnull()==False).astype(np.float64).replace(0, np.nan)
    ...
    PDI_ = PDI_.apply(lambda col: ta.PLUS_DI(h_[col.name], l_[col.name], c_[col.name], period_)).astype(np.float64)
    PDI_.replace(np.inf, 0, inplace = True)
    PDI_ = PDI_*data_exist    
    return PDI_    

def bollinger_band(target_: pd.DataFrame, period_: int, sd_up_, sd_down_: float)-> (pd.DataFrame, pd.DataFrame, pd.DataFrame):
    period_ = int(period_)
    data_exist = (target_.isnull()==False).astype(np.float64).replace(0, np.nan)
    data_exist = data_exist*(data_exist.shift(period_))
    target_ = clean_target(target_)     
    upper = target_.apply(lambda col: ta.BBANDS(col.astype(np.float64), period_, sd_up_, sd_down_, 0)[0]).astype(np.float64)   
    middle = target_.apply(lambda col: ta.BBANDS(col.astype(np.float64), period_, sd_up_, sd_down_, 0)[1]).astype(np.float64) 
    lower = target_.apply(lambda col: ta.BBANDS(col.astype(np.float64), period_, sd_up_, sd_down_, 0)[2]).astype(np.float64)   
    upper = upper*data_exist 
    middle = middle*data_exist 
    lower = lower*data_exist     
    return upper, middle, lower  

def MDI(h_: pd.DataFrame, l_: pd.DataFrame, c_: pd.DataFrame, period_: int)-> pd.DataFrame:
    period_ = int(period_)
    data_exist = (c_.isnull()==False).astype(np.float64).replace(0, np.nan)
    data_exist = data_exist*(data_exist.shift(6*period_ + 5))
    h_ = clean_target(h_)       
    ...
    MDI_ = MDI_.apply(lambda col: ta.MINUS_DI(h_[col.name], l_[col.name], c_[col.name], period_)).astype(np.float64)
    MDI_.replace(np.inf, 0, inplace = True)
    MDI_ = MDI_*data_exist    
    return MDI_    


def MACDold(target_: pd.DataFrame, fast_period_: int, slow_period_: int)-> pd.DataFrame:
    fast_period_ = int(fast_period_)
    slow_period_ = int(slow_period_)
    data_exist = (target_.isnull()==False).astype(np.float64).replace(0, np.nan)
    data_exist = data_exist*(data_exist.shift(slow_period_))
    target_ = clean_target(target_)       
    MACD_ = target_.apply(lambda col: ta.MACD(col, fast_period_, slow_period_, 9)[0]).astype(np.float64)
    MACD_.replace(np.inf, 0, inplace = True)
    MACD_ = MACD_*data_exist    
    return MACD_

def MFI(h_: pd.DataFrame, l_: pd.DataFrame, c_: pd.DataFrame, v_: pd.DataFrame, period_: int)-> pd.DataFrame:
    period_ = int(period_)
    data_exist = (c_.isnull()==False).astype(np.float64).replace(0, np.nan)
    data_exist = data_exist*(data_exist.shift(period_))
    h_ = clean_target(h_)       
    ...
    MFI_ = MFI_.apply(lambda col: ta.MFI(h_[col.name], l_[col.name], c_[col.name], v_[col.name], period_)).astype(np.float64)
...
def perK(h_: pd.DataFrame, l_: pd.DataFrame, target_: pd.DataFrame, range_: int, SMA_smooth: int)-> pd.DataFrame:
    range_ = int(range_)
    ...
    perK_ = perK_.apply(lambda col: ta.STOCH(h_[col.name], l_[col.name], target_[col.name], range_, SMA_smooth, 0, 3, 0)[0]).astype(np.float64)
...
def BarsSince_latest_trg(condition: pd.DataFrame) -> pd.DataFrame:
    ...
    BarsSince_latest_trg_index_ = pd.DataFrame(index = condition.index, columns = condition.columns, data = np.nan)
...
def BarsSince_first_trg(condition: pd.DataFrame) -> pd.DataFrame:
    ...
    BarsSince_first_trg_index_ = pd.DataFrame(index = condition.index, columns = condition.columns, data = np.nan)

    BarsSince_first_trg_index_[(condition == True)&(condition.shift().replace(np.nan, False) == False)] = 1.0
...
def BarsSince_since_first_buy_to_sell(BUY_: pd.DataFrame, SELL_: pd.DataFrame) -> pd.DataFrame:
    
    BUY_SELL_one_zero = one_zero_gen_frm_BUYSELL(BUY_, SELL_)

    index_np = np.arange(BUY_SELL_one_zero.index.size) 
    index_df = pd.DataFrame(index = BUY_SELL_one_zero.index, columns = BUY_SELL_one_zero.columns, data = 0)
    index_df = index_df.apply(lambda col: index_np)  

    BarsSince_first_trg_index_ = pd.DataFrame(index = BUY_SELL_one_zero.index, columns = BUY_SELL_one_zero.columns, data = np.nan)

    BarsSince_first_trg_index_[(BUY_SELL_one_zero == 1)&(BUY_SELL_one_zero.shift() == 0)] = 1.0
    BarsSince_first_trg_index_[(BUY_SELL_one_zero == 0)&(BUY_SELL_one_zero.shift() == 1)] = 0.0
    ...
    BarsSince_first_trg_ = index_df - BarsSince_first_trg_index_ + 1 # avoid bug dsc_oldest_ranking_DSC_within_universe

    return BarsSince_first_trg_



def mean_trim_within_universe(target_: pd.DataFrame, universe: pd.DataFrame, trim_: float = 0.10)->pd.DataFrame:

    from scipy import stats
    target_within_universe = target_*universe

    mean_ = target_within_universe.apply(lambda row: (stats.trim_mean(row[~np.isnan(row.astype(np.float64))], trim_)), axis = 1)
    mean_within_universe_ = pd.DataFrame(index = mean_.index, columns = target_.columns)
    mean_within_universe_ = mean_within_universe_.apply(lambda col: mean_.values).ffill(limit = 20)
...
def mean_within_universe(target_: pd.DataFrame, universe: pd.DataFrame)->pd.DataFrame:
    ...
    mean_ = target_within_universe.apply(lambda row: (row[~np.isnan(row.astype(np.float64))].mean()), axis = 1)
    mean_within_universe_ = pd.DataFrame(index = mean_.index, columns = target_.columns)
    mean_within_universe_ = mean_within_universe_.apply(lambda col: mean_.values).ffill(limit = 20)
    # mean_within_universe_ = mean_within_universe_*universe
    return mean_within_universe_.astype(np.float64)
...
def universe_excluding_odds(score_: pd.DataFrame, universe: pd.DataFrame, trim_ = 0.025)->pd.DataFrame:
    universe.replace(0, np.nan, inplace = True)
    score_within_universe = score_*universe
    odd_removed_universe = score_within_universe.apply(lambda row: remove_odds_from_row(row, trim_), axis = 1)
...
def winsorizing_within_universe(target_: pd.DataFrame, universe: pd.DataFrame, winsor_pecent_ = 0.025)->pd.DataFrame:
    universe.replace(0, np.nan, inplace = True)
    target_within_universe = target_*universe
    winsorizing_ = target_within_universe.apply(lambda row: winsorizing(row, winsor_pecent_), axis = 1)
    return winsorizing_.astype(np.float64)

def sd_within_universe(target_: pd.DataFrame, universe: pd.DataFrame)->pd.DataFrame:
    # Sample standard deviation : divided by n - 1
    target_within_universe = target_*universe
    sd_ = target_within_universe.apply(lambda row: (row[~np.isnan(row.astype(np.float64))].std()), axis = 1)
    sd_within_universe_ = pd.DataFrame(index = sd_.index, columns = target_.columns)
    sd_within_universe_ = sd_within_universe_.apply(lambda col: sd_.values).ffill().fillna(0)  
    # sd_within_universe_ = sd_within_universe_*universe
    return sd_within_universe_.astype(np.float64)

def z_score_within_universe(target_: pd.DataFrame, universe: pd.DataFrame)->pd.DataFrame:
...
def stability_within_universe(target_: pd.DataFrame, universe: pd.DataFrame)->pd.DataFrame:
...
def ATRold(h_: pd.DataFrame, l_: pd.DataFrame, c_: pd.DataFrame, period_: int)-> pd.DataFrame:
    period_ = int(period_)
    h_ = clean_target(h_)       
    l_ = clean_target(l_)       
    c_ = clean_target(c_) 
    ...
    ATR_ = ATR_.apply(lambda col: ta.ATR(h_[col.name], l_[col.name], c_[col.name], period_)).astype(np.float64)
...
def clean_shooting_dropping(target_: pd.DataFrame, mul_:float, period_ATR_: int, period_MA_: int) -> pd.DataFrame:
...
def delay_out_new(target_: pd.DataFrame, step_:float, period_: int) -> pd.DataFrame:
    ...
    target_ = target_.apply(lambda col: np.maximum(target_slow[col.name], target_fast[col.name]))    
    target_ = target_.apply(np.ceil)*step_
    return target_.astype(np.float64)


def dsc_oldest_ranking_DSC_within_universe(target_: pd.DataFrame, universe: pd.DataFrame, buy_top_largest: int, sell_drop_from_top: int, weight_recent_ranking = 0.11)->pd.DataFrame:
    # buy top smallest

    target_dsc_ranking = ranking_DSC_within_universe(target_, universe)
    BUY = (target_dsc_ranking <= buy_top_largest)&(target_dsc_ranking.shift() > buy_top_largest)
    SELL = (target_dsc_ranking >= sell_drop_from_top)&(target_dsc_ranking.shift() < sell_drop_from_top)
    ...
    bars_since_buy_ranking_dsc = ranking_DSC_within_universe(bars_since_buy, universe_frm_ranking).astype(np.float64)
    # include the following thing to avoid same ranking number
    weight_recent_ranking = weight_recent_ranking + 0.001
    adj_bars_since_buy_ranking_val_asc = bars_since_buy_ranking_dsc + weight_recent_ranking*target_dsc_ranking
    adj_bars_since_buy_ranking_asc = ranking_ASC_within_universe(adj_bars_since_buy_ranking_val_asc, universe_frm_ranking).astype(np.float64) 
...
def sig_ranking_DSC_within_universe(target_: pd.DataFrame, universe: pd.DataFrame, buy_top_largest: int, sell_drop_from_top: int, weight_recent_ranking = 0.11)->pd.DataFrame:

    dsc_oldest_ranking_DSC_within_universe_ = dsc_oldest_ranking_DSC_within_universe(target_, universe, buy_top_largest, sell_drop_from_top, weight_recent_ranking)
    
    adj_bars_since_buy_ranking_asc = dsc_oldest_ranking_DSC_within_universe_[dsc_oldest_ranking_DSC_within_universe_<=buy_top_largest]
    
    return adj_bars_since_buy_ranking_asc


def dsc_oldest_ranking_ASC_within_universe(target_: pd.DataFrame, universe: pd.DataFrame, buy_top_smallest: int, sell_drop_from_top: int, weight_recent_ranking = 0.01)->pd.DataFrame:
    # buy top smallest

    target_asc_ranking = ranking_ASC_within_universe(target_, universe)
    BUY = (target_asc_ranking <= buy_top_smallest)&(target_asc_ranking.shift() > buy_top_smallest)
    SELL = (target_asc_ranking >= sell_drop_from_top)&(target_asc_ranking.shift() < sell_drop_from_top)
    universe_frm_ranking = one_zero_gen_frm_BUYSELL(BUY, SELL)*universe
    universe_frm_ranking.replace(0, np.nan, inplace = True)
    ...
    # include the following thing to avoid same ranking number
    weight_recent_ranking = weight_recent_ranking + 0.001
    adj_bars_since_buy_ranking_val_asc = bars_since_buy_ranking_dsc + weight_recent_ranking*target_asc_ranking
    adj_bars_since_buy_ranking_asc = ranking_ASC_within_universe(adj_bars_since_buy_ranking_val_asc, universe_frm_ranking).astype(np.float64) 
...
def sig_ranking_ASC_within_universe(target_: pd.DataFrame, universe: pd.DataFrame, buy_top_smallest: int, sell_drop_from_top: int, weight_recent_ranking = 0.11)->pd.DataFrame:
    # buy top smallest
    dsc_oldest_ranking_ASC_within_universe_ = dsc_oldest_ranking_ASC_within_universe(target_, universe, buy_top_smallest, sell_drop_from_top, weight_recent_ranking)
    
    adj_bars_since_buy_ranking_asc = dsc_oldest_ranking_ASC_within_universe_[dsc_oldest_ranking_ASC_within_universe_<=buy_top_smallest]
...
def normalize_row_by_perK(target_: pd.DataFrame, universe_: pd.DataFrame)->pd.DataFrame:
    exist = data_exist_1_nan(target_)
    ...
    output[(HHV_df - LLV_df) == init_df(target_, 0)] = 1.0 # case only one existing value in a row
    output[(HHV_df - LLV_df) == init_df(target_, np.nan)] = 1.0
    output[(HHV_df == init_df(target_, np.nan))|(LLV_df == init_df(target_, np.nan))] = 0.00
...
def convert_weight_to_share(weight_: pd.DataFrame, price_: pd.DataFrame, deposit_: float) -> pd.DataFrame:
...
def txn_reduction__change_share_only_when_weight_change(shares_: pd.DataFrame, weight_: pd.DataFrame, allow_thres_: float)->pd.DataFrame:
...
def txn_reduction__change_share_only_new_sig(shares_: pd.DataFrame, weight_: pd.DataFrame)->pd.DataFrame:
...
def cap_maximal_value(target_: pd.DataFrame, max_value_ = 0.10) -> pd.DataFrame:
    ...
    return target_.apply(lambda col: np.minimum(col, max_df[col.name])).astype(np.float64)

def cap_minimal_value(target_: pd.DataFrame, min_value_ = -0.10) -> pd.DataFrame:
    min_df = init_df(target_, min_value_)
    return target_.apply(lambda col: np.maximum(col, min_df[col.name])).astype(np.float64)
...
def allow_only_shares_decline(shares_: pd.DataFrame, percent_chg_from_original = 10, percent_chg_from_prev = 5) -> pd.DataFrame:
    ...
    output_[(shares_  < original_shares)&(shares_/shares_.shift() -1.0 < -1.0*percent_chg_from_prev/100)&(shares_/original_shares - 1.0 < -1.0*percent_chg_from_original/100.)] = int(1)
...
def even_better_sinewave(target_: pd.DataFrame, duration_ = 20):
    ...
    for row_i in range(2, target_.shape[0]):
        ...
        HP[row_i, :] = .5*(1 + alpha1)*(target_np[row_i, :] - target_np[row_i - 1, :] ) + alpha1* HP[row_i - 1, :]
    
        Filt[row_i, :] = c1*(HP[row_i, :] + HP[row_i - 1, :]) / 2 + c2*Filt[row_i - 1, :]  + c3*Filt[row_i - 2, :]
        
    Filt = pd.DataFrame(index = target_.index, columns = target_.columns, data = Filt)
        
    Wave = (Filt + Filt.shift() + Filt.shift(2)) / 3
    Pwr = (Filt*Filt + Filt.shift()*Filt.shift() + Filt.shift(2)*Filt.shift(2)) / 3
...
def display_comparison_2_axis(stock_name: str, target_01_: pd.Series, target_01_name_: str, target_02_: pd.Series, target_02_name_: str, time_from = '2014-01-01', time_end = '2021-01-01'):
    '''
    ...
    ax.plot(target_01_[time_from:time_end].index, target_01_[time_from:time_end].values, label = target_01_name_)
    ax.legend(loc ="upper left")
   
    # twin object for two different y-axis on the sample plot
    ax2=ax.twinx()
    # make a plot with different y-axis using second axis object
    ax2.plot(target_02_[time_from:time_end].index, target_02_[time_from:time_end].values,color="red", label = target_02_name_) #  alpha=0.3
    # ax2.plot(drawdown_percent_NAV.index, drawdown_percent_NAV.values,color="orange", label = 'DD_NAV')
    ax2.legend(loc ="upper right")
    fig.text(.5, .05, stock_name + ': ' + target_01_name_ + ' vs ' + target_02_name_, ha = 'center', va = 'bottom')
    
    
def display_comparison_2_axis_left_list(stock_name: str, target_01_list: list, target_01_name_: str, target_02_: pd.Series, target_02_name_: str, time_from = '2014-01-01', time_end = '2021-01-01'):
    '''
    ...
    for item in target_01_list:
        ...
        ax.plot(item[time_from:time_end].index, item[time_from:time_end].values, label = target_01_name_)
...
def df_pivot_ranking_by_alphabet(target_):
    ...
    target_melt_pivot =  target_melt.pivot(values='Ticker', columns='rank', index = 'index') 
    target_melt_pivot_alphabet = target_melt_pivot.apply(lambda row: row.sort_values(), axis = 1)
...
def get_avg_all__avg_m__avg_p__var_m__var_p__es_m__es_p__max_m__max_p(target_: pd.DataFrame, period_ = 240, confident_percent = 0.05):
    ...
    avg_all_np, avg_m_np, avg_p_np, var_m_np, var_p_np, es_m_np, es_p_np, max_m_np, max_p_np = init_np(target_np, np.nan) \
                                                 ,init_np(target_np, np.nan), init_np(target_np, np.nan), init_np(target_np, np.nan)\
                                                 ,init_np(target_np, np.nan), init_np(target_np, np.nan), init_np(target_np, np.nan)\
                                                 ,init_np(target_np, np.nan), init_np(target_np, np.nan)
    ...
    for stock in range(stock_num):
        ...
        for row in range(period_-1 ,bar_num ,1): 
            ...
            if(np.isnan(sliced_np).sum() < 1):  
                avg_all_np[row, stock], avg_m_np[row, stock], avg_p_np[row, stock]\
                    , var_m_np[row, stock], var_p_np[row, stock], es_m_np[row, stock], es_p_np[row, stock], max_m_np[row, stock], max_p_np[row, stock] \
                          = cal_avg_all__avg_m__avg_p__var_m__var_p__es_m__es_p__max_m__max_p_from_np(sliced_np, confident_percent)          
    
    avg_all_df_, avg_m_df_, avg_p_df_, var_m_df_, var_p_df_, es_m_df_, es_p_df_, max_m_df_, max_p_df_ = np_to_df(avg_all_np, target_) \
                                                ,np_to_df(avg_m_np, target_), np_to_df(avg_p_np, target_), np_to_df(var_m_np, target_)\
                                                ,np_to_df(var_p_np, target_), np_to_df(es_m_np, target_), np_to_df(es_p_np, target_)\
                                                ,np_to_df(max_m_np, target_), np_to_df(max_p_np, target_)

    return (avg_all_df_, avg_m_df_, avg_p_df_, var_m_df_, var_p_df_, es_m_df_, es_p_df_, max_m_df_, max_p_df_ )



def cal_avg_all__avg_m__avg_p__var_m__var_p__es_m__es_p__max_m__max_p_from_np(target_: np.array, confident_percent_ = 0.05) ->(pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame):
...
def cal_avg_all__avg_m__avg_p__var_m__var_p__es_m__es_p__max_m__max_p_from_series(target_: pd.Series, confident_percent_ = 0.05) ->(pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame):
...
def np_to_df(mother_: np.array, df_blueprint_: pd.DataFrame):  
    return pd.DataFrame(index = df_blueprint_.index, columns = df_blueprint_.columns, data = mother_)  
...
def ts_rank_pct(input_df: pd.DataFrame, period_)->pd.DataFrame:
    ...
    period_ = int(period_)
    return input_df.rolling(period_).apply(lambda row: rollingRankOnSeries(row)/period_)
...
def get_downside_deviationold(target_: pd.DataFrame, period_ = 60):
    ...
    downside_deviation_ = pd.DataFrame(index = target_.index, columns = target_.columns, data = downside_deviation_np)
...
def gen_target_shares_bool_sig_equal_weight(BUY_: pd.DataFrame, SELL_: pd.DataFrame,trade_universe_: pd.DataFrame, c_: pd.DataFrame, setting_backtest_: dict, method = 'one_way', cap_weight_ = 0.25):
    ...
    target_shares = convert_weight_to_share(weight, c_, setting_backtest_['initial_deposit'])
...
def clean_weight_over_1(target_weight):
    ...
    target_weight[target_sum > 1.0000000000000] = target_weight[target_sum > 1.0000000000000]/target_sum_extend[target_sum > 1.0000000000000] 
